...
R1_L, R2_L, P1_L, P2_L, Q_L, validPix_l1, validPix_l2 = cv2.stereoRectify(camera_L_mtx, dist_L, camera_R_mtx, dist_R, size_L, R, T) # for cam L R1 矫正变换旋转矩阵
R1_R, R2_R, P1_R, P2_R, Q_R, validPix_r1, validPix_r2 = cv2.stereoRectify(camera_R_mtx, dist_R, camera_L_mtx, dist_L, size_L, R, T) # for cam R P1 新坐标系下投影阵
...
right_map1, right_map2 = cv2.initUndistortRectifyMap(camera_R_mtx, dist_R, R1_L, P1_L, size_L, cv2.CV_16SC2) # for cam R
...
cv2.resizeWindow("right", 600, 600)
...
while True:
    ...
    disparity = stereo.compute(img_L_gray, img_R_gray) #两幅图像重叠在一起的时候，左摄像机上P的投影和右摄像机上P的投影位置有
